(() => {
  ...
  function titleMap(view){return {
    ...
    listening:['聽力題庫',`${LISTENING_QUESTIONS.length} 題看圖聽解真題：圖片、音檔、發話選項與深度解析完整保留。`],
  ...
  function renderDashboard(){
    ...
    document.getElementById('view-dashboard').innerHTML=`
      ...
      <div class="card hero">
        ...
        <p>目前已整理 ${KNOW.length} 個核心知識點，並加入 ${LISTENING_QUESTIONS.length} 題真正的看圖聽解真題。刷題與聽力都會累積 EXP、Combo 與複習紀錄；系統課程與間隔複習則負責把短期答對變成真正掌握。</p>
      ...
      <div class="grid mode-grid">
        ...
        ${modeCard('listening','聽力題庫',`${LISTENING_QUESTIONS.length} 題看圖聽解真題`,'圖片與音檔是題目本體；作答前不顯示發話文字，答題後再解鎖完整解析。')}
  ...
  function listeningPackById(id){return LISTENING_PACKS.find(p=>p.id===id)||LISTENING_PACKS[0]||null}
  function listeningPackQuestions(packId){const pack=listeningPackById(packId);if(!pack)return[];return (pack.questionIds||[]).map(id=>LQMAP[id]).filter(Boolean).sort((a,b)=>(a.number||0)-(b.number||0))}
  ...
  function startListeningSession(mode='all',packId=LISTENING_PACKS[0]?.id){const pack=listeningPackById(packId);if(!pack){toast('目前沒有聽力題庫。');return}let list=listeningPackQuestions(pack.id);if(mode==='unanswered')list=list.filter(q=>(listeningProgressOf(q.id).attempts||0)===0);if(mode==='weak')list=listeningWeakQuestions(list);if(mode==='due')list=listeningDueQuestions(list);if(mode==='random')list=shuffle(list);if(!list.length){toast(mode==='unanswered'?'這個題組目前沒有未作答題。':mode==='weak'?'目前沒有聽力錯題。':mode==='due'?'目前沒有到期聽力複習。':'目前沒有可用聽力題。');return}let startIndex=0;if(mode==='continue'&&state.listening.lastQuestionId){const i=list.findIndex(q=>q.id===state.listening.lastQuestionId);if(i>=0)startIndex=i}listeningSession={mode,packId:pack.id,ids:list.map(q=>q.id),index:startIndex,correct:0,wrong:0,records:{}};state.listening.lastPackId=pack.id;saveState();switchView('listening')}
