(() => {
  ...
  function totalStats(){
    ...
    QUESTIONS.forEach(q=>{const p=progressOf(q.id);attempts+=p.attempts||0;correct+=p.correct||0;if(p.attempts)answered++;if((p.streak||0)>=3)mastered++;if((p.wrong||0)>0)wrongSet++;});
  ...
  function listeningStats(list=LISTENING_QUESTIONS){let attempts=0,correct=0,answered=0,mastered=0,firstListenCorrect=0,relistenCorrect=0,totalPlays=0;list.forEach(q=>{const p=listeningProgressOf(q.id);attempts+=p.attempts||0;correct+=p.correct||0;totalPlays+=p.totalPlays||0;firstListenCorrect+=p.firstListenCorrect||0;relistenCorrect+=p.relistenCorrect||0;if(p.attempts)answered++;if((p.streak||0)>=3)mastered++;});return {attempts,correct,answered,mastered,firstListenCorrect,relistenCorrect,totalPlays,accuracy:pct(correct,attempts),firstListenAccuracy:pct(firstListenCorrect,attempts)}}
  ...
  function renderDashboard(){
    const st=totalStats(); const sys=systemStats(); const lst=listeningStats(); const orig=QUESTIONS.filter(q=>q.source==='原題').length; const business=BUSINESS_QUESTIONS.length; const ext=QUESTIONS.filter(q=>q.source==='延伸').length; const due=dueQuestions().length; const weak=weakQuestions().length;
    ...
    document.getElementById('view-dashboard').innerHTML=`
      ...
      <div class="grid stats-grid">
        ...
        <div class="card stat"><span>聽力真題</span><strong>${lst.answered}/${LISTENING_QUESTIONS.length}</strong><small>正確率 ${lst.accuracy}% · 初聽正解 ${lst.firstListenAccuracy}%</small></div>
      </div>
      ...
      <div class="card hero">
        ...
        <p>目前已整理 ${KNOW.length} 個核心知識點，並加入 ${LISTENING_QUESTIONS.length} 題真正的聽解／聽讀解真題。刷題與聽力都會累積 EXP、Combo 與複習紀錄；系統課程與間隔複習則負責把短期答對變成真正掌握。</p>
        <div class="quick-actions">
          ...
          <button class="btn" id="dashListeningBtn">🎧 聽力題庫 (${LISTENING_QUESTIONS.length})</button>
      ...
      <div class="grid mode-grid">
        ...
        ${modeCard('listening','聽力題庫',`${LISTENING_QUESTIONS.length} 題聽解／聽讀解真題`,'圖片與音檔是題目本體；有來源詳解的題目提供深度解析，只有答案表的題目則明確標示為正解驗證，不臆測逐字稿。')}
  ...
  function renderLessonDetail(l){
    ...
    const kids=validLessonKnowledgeIds(l), aids=validLessonArticleIds(l), lids=validLessonListeningIds(l), qids=validLessonQuestionIds(l), practiceIds=lessonPracticeIds(l), sectionDefs=lessonSectionDefs(l);
  ...
  function bindStartButtons(){document.querySelectorAll('[data-start]').forEach(b=>b.addEventListener('click',()=>{const m=b.dataset.start;if(m==='system'){switchView('system');return}if(m==='knowledge'){switchView('knowledge');return}if(m==='business'){switchView('business');return}if(m==='listening'){switchView('listening');return}startSession(m)}))}
  ...
  function startListeningSession(mode='all',packId=LISTENING_PACKS[0]?.id){const allScope=packId==='__ALL__';const pack=allScope?null:listeningPackById(packId);if(!allScope&&!pack){toast('目前沒有這個聽力題組。');return}let list=listeningPackQuestions(allScope?'__ALL__':pack.id);if(mode==='unanswered')list=list.filter(q=>(listeningProgressOf(q.id).attempts||0)===0);if(mode==='weak')list=listeningWeakQuestions(list);if(mode==='due')list=listeningDueQuestions(list);if(mode==='random'||mode==='random20')list=shuffle(list);if(mode==='random20')list=list.slice(0,20);if(!list.length){toast(mode==='unanswered'?'目前沒有未作答聽力題。':mode==='weak'?'目前沒有聽力錯題。':mode==='due'?'目前沒有到期聽力複習。':'目前沒有可用聽力題。');return}let startIndex=0;if(mode==='continue'&&state.listening.lastQuestionId){const i=list.findIndex(q=>q.id===state.listening.lastQuestionId);if(i>=0)startIndex=i}listeningSession={mode,packId:allScope?'__ALL__':pack.id,scope:allScope?'global':'pack',ids:list.map(q=>q.id),index:startIndex,correct:0,wrong:0,records:{}};if(!allScope)state.listening.lastPackId=pack.id;saveState();switchView('listening')}
  function startListeningOne(id){const q=LQMAP[id];if(!q){toast('找不到對應聽力題。');return}listeningSession={mode:'one',packId:q.packId,ids:[id],index:0,correct:0,wrong:0,records:{}};state.listening.lastPackId=q.packId;state.listening.lastQuestionId=id;saveState();switchView('listening')}
  ...
  function openListeningMap(){if(!listeningSession)return;const modal=document.getElementById('questionMapModal');if(!modal)return;modal.hidden=false;modal.innerHTML=`<div class="question-map-card"><div class="question-map-head"><div><b>聽力題目一覽</b><span>${listeningSession.ids.length} 題</span></div><button class="btn" id="closeListeningMap">關閉</button></div><div class="question-map-grid">${listeningSession.ids.map((id,i)=>{const r=listeningSession.records?.[String(i)];const p=listeningProgressOf(id);const status=r?.answered?(r.correct?'correct':'wrong'):(p.attempts?(p.lastCorrect?'correct':'wrong'):'unanswered');return `<button class="question-map-item ${status} ${i===listeningSession.index?'current':''}" data-listen-map-index="${i}">${i+1}<small>${status==='correct'?'✓':status==='wrong'?'✕':'○'}</small></button>`}).join('')}</div><div class="question-map-legend"><span>✓ 已正解</span><span>✕ 最近錯誤</span><span>○ 未作答</span><span>▶ 現在</span></div></div>`;document.getElementById('closeListeningMap').onclick=()=>{modal.hidden=true;modal.innerHTML=''};modal.onclick=e=>{if(e.target===modal){modal.hidden=true;modal.innerHTML=''}};modal.querySelectorAll('[data-listen-map-index]').forEach(b=>b.onclick=()=>{modal.hidden=true;modal.innerHTML='';listeningGoTo(Number(b.dataset.listenMapIndex))})}
  ...
  function renderListeningQuestion(root){const q=listeningCurrentQ(),rec=listeningRecord();if(!q||!rec){renderListeningSessionEnd(root);return}state.listening.lastQuestionId=q.id;const p=listeningProgressOf(q.id),answer=Number(q.correctAnswer?.originalIndex),pack=listeningPackById(q.packId),deep=q.reviewMode!=='answer_key_only',rule=q.dataRule||(deep?'圖片與音檔都是題目本體；作答前不顯示四個發話文字。':'圖片與音檔都是題目本體；依來源可驗證資料作答。');const optionLabel=n=>{const o=(q.options||[]).find(x=>Number(x.originalIndex)===n);if(!rec.answered)return deep?'發話選項':'選項';if(o?.text)return o.text;if(n===answer)return'正解';if(n===rec.selectedOriginalIndex)return'你的答案';return`選項 ${n}`};root.innerHTML=`<div class="listening-practice-layout"><main class="card listening-question-card"><div class="practice-head"><span class="q-number">🎧 ${esc(pack?.titleZh||pack?.title||'聽力真題')} · ${listeningSession.index+1} / ${listeningSession.ids.length}</span><span class="q-source">${esc(q.difficulty||'BJT')}</span></div><div class="progress"><i style="width:${pct(listeningSession.index,listeningSession.ids.length)}%"></i></div><div class="listening-visual-warning"><b>🖼️ 此題必須看圖＋聽音檔作答</b><span>${esc(rule)}</span><small id="listeningVisualStatus">圖片載入中；圖片與音檔都就緒後才可作答。</small></div><button class="listening-image-button" data-image-viewer="${esc(q.media?.image||'')}" data-image-alt="${esc(q.id)} 題目圖片"><img src="${esc(q.media?.image||'')}" alt="${esc(q.id)} 題目圖片" loading="eager"><span>🔍 點擊放大題目圖片</span></button><section class="listening-player"><div class="listening-player-head"><div><span class="lesson-kicker">${q.category==='聴読解'?'LISTENING + READING':'LISTENING'}</span><b>${esc(q.category||'聽力')}</b></div><span id="listeningPlayCount">播放 ${rec.playCount||0} 次</span></div><audio id="listeningAudio" src="${esc(q.media?.audio||'')}" preload="metadata"></audio><div class="audio-main-controls"><button class="btn primary" id="listeningPlay">▶ 播放</button><button class="btn" id="listeningReplay">↺ 從頭重播</button><button class="btn" id="listeningBack5">−5秒</button><button class="btn" id="listeningForward5">+5秒</button></div><div class="audio-seek-row"><span id="listeningTime">00:00 / ${formatAudioTime(q.audioDurationSec||0)}</span><input id="listeningSeek" type="range" min="0" max="${Number(q.audioDurationSec||1)}" step="0.1" value="0"><select id="listeningSpeed" class="select"><option value="0.75">0.75×</option><option value="1" selected>1.0×</option><option value="1.25">1.25×</option></select></div></section><section class="listening-prompt"><span class="lesson-kicker">QUESTION</span><h2>${esc(q.questionZhTW||q.question)}</h2><p>${esc(q.question)}</p></section><div class="listening-number-options">${[1,2,3,4].map(n=>`<button class="listening-number-option ${rec.answered&&n===answer?'correct':''} ${rec.answered&&n===rec.selectedOriginalIndex&&n!==answer?'wrong':''}" data-listening-answer="${n}" ${!rec.audioStarted||rec.answered?'disabled':''}><span>${n}</span><small>${esc(optionLabel(n))}</small></button>`).join('')}</div>${!rec.audioStarted?'<div class="listen-first-hint">▶ 先播放音檔，之後才可選擇 1～4。</div>':''}<div class="question-navigation"><button class="btn" id="listenPrevBtn" ${listeningSession.index<=0?'disabled':''}>← 上一題</button><button class="btn" id="listenMapBtn">題目一覽</button><span class="question-nav-count">${listeningSession.index+1} / ${listeningSession.ids.length}</span><button class="btn primary" id="listenNextBtn">${listeningSession.index+1>=listeningSession.ids.length?'完成本輪':'下一題 →'}</button></div>${!rec.answered?'<div class="skip-hint">練習模式可先跳過未作答題，不會自動判錯。</div>':''}${rec.answered?renderListeningFeedback(q,rec):''}</main><aside class="grid"><div class="card side-card"><h3>聽力學習狀態</h3><div class="mini-list"><div class="mini-item"><b>作答 ${p.attempts||0} 次</b><small>答對 ${p.correct||0}／答錯 ${p.wrong||0}／連對 ${p.streak||0}</small></div><div class="mini-item"><b>初聽正解 ${p.firstListenCorrect||0}</b><small>重聽後正解 ${p.relistenCorrect||0} · 累積播放 ${p.totalPlays||0} 次</small></div></div></div><div class="card side-card"><h3>這個題組的資料層級</h3><div class="listening-quality ${deep?'deep':'verified'}">${deep?'深度解析':'正解已驗證'}</div><p class="small">${deep?'這個題組有來源可核對的發話文字、選項解析與教學說明。':'這個題組已驗證題目圖、作答音檔與正解；來源未提供可靠完整逐字稿／詳解，所以不補寫未驗證內容。'}</p></div></aside></div>`;document.querySelectorAll('[data-listening-answer]').forEach(b=>b.onclick=()=>answerListening(Number(b.dataset.listeningAnswer)));document.getElementById('listenPrevBtn').onclick=listeningPrev;document.getElementById('listenNextBtn').onclick=listeningNext;document.getElementById('listenMapBtn').onclick=openListeningMap;document.querySelectorAll('[data-listen-jump]').forEach(b=>b.onclick=()=>jumpListeningFeedback(b.dataset.listenJump));bindListeningAudio(q,rec)}
  function renderListeningSessionEnd(root){const total=listeningSession.correct+listeningSession.wrong,unanswered=Math.max(0,listeningSession.ids.length-total),global=listeningSession.packId==='__ALL__';root.innerHTML=`<section class="card hero listening-end"><span class="player-kicker">LISTENING COMPLETE</span><h2>本輪聽力完成</h2><p>答對 ${listeningSession.correct}／${total}，正確率 ${pct(listeningSession.correct,total)}%。${unanswered?`另有 ${unanswered} 題保留未作答。`:''}聽力錯題與到期複習會獨立追蹤，不會混亂原本 416 題文字題的統計。</p><div class="quick-actions">${unanswered?'<button class="btn primary" id="listenResume">回到未作答題</button>':''}<button class="btn" id="listenAgain">再練本範圍</button><button class="btn" id="listenWeak">全題庫錯題再戰</button><button class="btn" id="listenHome">回聽力首頁</button></div></section>`;const resume=document.getElementById('listenResume');if(resume)resume.onclick=()=>{const idx=listeningSession.ids.findIndex((_,i)=>!listeningSession.records?.[String(i)]?.answered);listeningGoTo(idx>=0?idx:0)};document.getElementById('listenAgain').onclick=()=>startListeningSession('all',global?'__ALL__':listeningSession.packId);document.getElementById('listenWeak').onclick=()=>{listeningSession=null;startListeningSession('weak','__ALL__')};document.getElementById('listenHome').onclick=()=>{listeningSession=null;renderListening()}}
  function renderListeningHome(root){const all=listeningStats(),due=listeningDueQuestions().length,weak=listeningWeakQuestions().length,lastId=state.listening.lastQuestionId;const renderPackCard=pack=>{const list=listeningPackQuestions(pack.id),st=listeningStats(list),deep=pack.reviewLevel==='deep';return `<article class="card listening-pack-card"><div class="listening-pack-top"><div><div class="pack-badges"><span class="tag">${esc(pack.difficulty||'BJT')}</span><span class="listening-quality ${deep?'deep':'verified'}">${esc(pack.dataQualityLabel||'')}</span></div><h3>${esc(pack.titleZh||pack.title)}</h3></div><b>${list.length} 題</b></div><p>${esc(pack.description||'')}</p><div class="progress"><i style="width:${pct(st.answered,list.length)}%"></i></div><div class="listening-pack-meta"><span>${st.answered}/${list.length} 已完成</span><span>正確率 ${st.accuracy}%</span><span>初聽 ${st.firstListenAccuracy}%</span></div><div class="actions"><button class="btn primary" data-listen-pack="${esc(pack.id)}">開始題組</button><button class="btn" data-listen-random="${esc(pack.id)}">隨機練習</button></div></article>`};const legacy=LISTENING_PACKS.filter(p=>p.collectionId==='BJT_JPLUS_ORIGINAL');const jp02=LISTENING_PACKS.filter(p=>p.collectionId==='BJT_JPLUS02');const part1=jp02.filter(p=>p.part===1),part2=jp02.filter(p=>p.part===2);root.innerHTML=`<section class="card listening-hero"><div><span class="player-kicker">LISTENING TRAINING</span><h2>聽解 × 聽讀解 × 商務語用</h2><p>目前共 ${LISTENING_QUESTIONS.length} 題。不同來源包依可驗證資料分成「深度解析」與「正解確認済」兩種層級；圖片與音檔一律視為題目本體。</p></div><div class="listening-hero-count"><strong>${LISTENING_QUESTIONS.length}</strong><span>真題</span></div></section><div class="grid stats-grid listening-stats"><div class="card stat"><span>已完成</span><strong>${all.answered}/${LISTENING_QUESTIONS.length}</strong><small>${all.mastered} 題已熟練</small></div><div class="card stat"><span>聽力正確率</span><strong>${all.accuracy}%</strong><small>${all.correct} / ${all.attempts} 次作答</small></div><div class="card stat"><span>初聽正解率</span><strong>${all.firstListenAccuracy}%</strong><small>第一次播放就答對 ${all.firstListenCorrect} 次</small></div><div class="card stat"><span>待處理</span><strong>${weak}</strong><small>錯題 · ${due} 題到期複習</small></div></div><div class="quick-actions listening-actions">${lastId?'<button class="btn primary" data-listen-start="continue">▶ 繼續上次</button>':''}<button class="btn" data-listen-global="random20">全題庫隨機 20 題</button><button class="btn" data-listen-global="unanswered">全題庫未作答</button><button class="btn ${weak?'bad':''}" data-listen-global="weak">全題庫錯題 (${weak})</button><button class="btn ${due?'warn':''}" data-listen-global="due">全題庫到期 (${due})</button></div><section class="listening-library-group"><div class="section-title"><div><h2>深度解析題組</h2><p>來源提供可核對的選項文字與詳解；答題後解鎖完整解析。</p></div></div><div class="listening-pack-grid">${legacy.map(renderPackCard).join('')}</div></section><section class="listening-library-group"><div class="section-title"><div><h2>BJT J+ 02｜65 題完整題組</h2><p>來源影片拆成第1部「聴解テスト」35 題與第2部「聴読解テスト」30 題。65 題都有獨立音檔、清理後題目圖與驗證正解；來源未提供可靠完整逐字稿／詳解，所以不臆測補寫。</p></div></div><div class="listening-part-block"><div class="listening-part-head"><div><span class="player-kicker">PART 1</span><h3>第1部｜聴解テスト</h3></div><b>35 題</b></div><div class="listening-pack-grid">${part1.map(renderPackCard).join('')}</div></div><div class="listening-part-block"><div class="listening-part-head"><div><span class="player-kicker">PART 2</span><h3>第2部｜聴読解テスト</h3></div><b>30 題</b></div><div class="listening-pack-grid">${part2.map(renderPackCard).join('')}</div></div></section>`;root.querySelectorAll('[data-listen-start]').forEach(b=>b.onclick=()=>startListeningSession(b.dataset.listenStart,state.listening.lastPackId||LISTENING_PACKS[0]?.id));root.querySelectorAll('[data-listen-global]').forEach(b=>b.onclick=()=>startListeningSession(b.dataset.listenGlobal,'__ALL__'));root.querySelectorAll('[data-listen-pack]').forEach(b=>b.onclick=()=>startListeningSession('all',b.dataset.listenPack));root.querySelectorAll('[data-listen-random]').forEach(b=>b.onclick=()=>startListeningSession('random',b.dataset.listenRandom))}
