(() => {
  ...
  function renderStemUnderstanding(q){
    ...
    return `<section class="stem-understanding" id="stemUnderstanding"><div class="stem-understanding-head"><div><span class="lesson-kicker">QUESTION UNDERSTANDING</span><h3>📘 題幹理解</h3></div></div><div class="stem-understanding-block"><b>日文原文</b><p class="stem-ja">${esc(q?.stem||'')}</p></div><div class="stem-understanding-block primary"><b>中文意思</b><p>${zh?esc(zh):'目前來源資料沒有可可靠核對的題幹中文；本題不自行補寫。'}</p></div><div class="stem-understanding-block"><b>這題真正要判斷的是</b><p>${esc(stemIntent(q))}</p></div>${cues.length?`<div class="stem-cue-list">${cues.map(x=>`<span>${esc(x)}</span>`).join('')}</div>`:''}<div class="stem-trap"><b>解題提醒</b><p>${esc(stemTrap(q))}</p></div></section>`;
  }
