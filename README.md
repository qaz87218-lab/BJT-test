# v14.1.1 — Listening Book 03 Media Realignment Hotfix

針對第3冊媒體錯位重新稽核並重切 **全部 65 題**，不是只修第1、2題。

- 音檔不再使用逐字稿中的約略時間直接切割；改以原始 M4A 的實際停頓邊界定位每題。
- 每題音訊統一從題目前停頓結束前 0.12 秒開始，到題後停頓開始後 0.12 秒結束，避免少念最後選項或吃到下一題。
- 65/65 音檔全部重新輸出。
- 65/65 題圖全部從與 M4A 同長度、同步的原始 MP4 重新擷取。
- 視覺稽核確認五個 Section 的題號與圖片依序對齊：10 / 10 / 15 / 15 / 15。
- 第1題舊版 45.00–68.75 秒已改為 46.774–73.314 秒；第2題舊版 69.00–99.75 秒已改為 76.107–100.794 秒。
- 65 題每一個切點都落在原始音檔的停頓區間內；起點與停頓結束差 -0.12 秒、終點與下一段停頓開始差 +0.12 秒。
- 題目 ID、正解、逐字稿、假名、繁中、Learning、前兩冊與 localStorage 均不改。
- Service Worker cache 更新為 v14.1.1，避免 GitHub Pages 繼續讀取錯位的 v14.1 媒體快取。
