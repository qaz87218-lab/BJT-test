# v14.5：BJT 模擬試驗 第2回・第3回
- 在既有第1回之外新增第2回、第3回，各80題。
- 每回固定：聴解25（5+10+10）／聴読解25（5+10+10）／読解30（10+10+10）。
- 三回皆維持：全程不可回頭、第1/2部音聲各題只能播放一次、分部限時45/30/30分鐘、完成後才顯示詳解與0–800 IRT-style模擬尺度分數。
- 模擬題皆為原創，不混入真題或延伸題。
- 成績歷史會記錄是哪一回，三回可重複挑戰。
...
# v14.4 — BJT 模擬試驗 第1回（2026-09-26）
- 新增獨立「📝 BJT 模擬試驗」區域，不混入真題、延伸題或聽力真題進度。
- 第1回共 80 題：聴解 25（5+10+10）、聴読解 25（5+10+10）、読解 30（10+10+10）。
- 部門計時：45 分／30 分／30 分，部門間無休息；時間到自動推進。
- 第1、2部每題音聲僅可播放一次，使用瀏覽器日文 Speech Synthesis；播放後立即鎖定，不可重播。
- 依使用者要求採更嚴格的「三部皆不可返回前題」模式。正式 BJT 規則是第1、2部不可返回，第3部在30分鐘內可返回確認。
- 考試中不顯示正解、翻譯、逐字稿或詳解；完成後才開放考後詳解。
- 新增 0–800 的 IRT-style 模擬尺度分數、官方 J5～J1+ 分數帶、使用者自訂 400 目標線及三部正確率。
- 注意：正式 BJT 分數由官方以經校準試題的 IRT 統計換算；本 App 分數只使用本回自製題目的內部難度參數估計，不能視為官方成績。
- 模擬試驗使用獨立 localStorage `bjtMockExamV1`，不影響既有 `bjtDeepStateV1`。
- 修正 v14.3 `index.html` 中 knowledge expansion / targeted extension script 被重複載入的問題。
- Service Worker cache 更新為 v14.4。

# v14.3 — Knowledge + Targeted Extension Audit（2026-09-24）

- 以 v14.2 的 606 題（416 文字＋190 聽力）為母體進行第二輪選項級知識整理。416 題文字的 1,664 個選項直接由題庫與 option details 校閱；190 題聽力的 760 個選項則以題目 typed choices 或各冊 Learning v3 的 optionAnalysis 還原，因此本輪可逐項檢視的選項位置合計 2,424 個。
- 新增 60 張細粒度知識卡：依頼・許可・報告確度・電話・會議反論・在庫／會計／営業等。
- 知識卡總數：166 → 226。
- 為 v14.2 新增的 48 張卡全部補 1 題 targeted extension。
- 為本版新增的 60 張卡全部補 1 題 targeted extension。
- 本版新增延伸題 108 題；延伸題總數：118 → 226。
- 新增延伸題全部有四選項獨立詳解，共新增 432 個 option details。
- 新增卡均有來源題 sourceRefs；可從知識卡直接「只刷這個知識點」。
- 原題、實用商務原題、聽力題、答案、圖片、音檔與 localStorage key 均不改寫。
...
# v14.2 — Knowledge Base Expansion (2026-09-24)

- 全面盤點目前 416 題文字題（1,664 個選項）與 190 題聽力題（760 個選項），合計 2,424 個選項位置。
- 知識卡由 118 張擴充為 166 張：新增 48 張「干擾選項／文法辨析／商務表現」知識卡。
- 額外深度補強 13 張既有知識卡，將原本只寫正解概念的卡片擴展為多選項辨析。
- 新增主題包含：Vる／Vている／Vたところ、Vようとする／Vたがる、Vたい／Vてほしい、～んだけど、に応じて、～ながら、にも／には、～って、～ごろ／まで／までに、なお、～ませんでしょうか、承知／了解／わかる、伝言を承る、ぜひとも、敬語稱謂、身內敬語、～ざるを得ない、見込み／見当／目当て／目算等。
- 知識卡可顯示「來源題」欄位，讓使用者知道此辨析來自哪一批原題／聽力題。
- 只有確實綁定文字題標籤的知識卡才顯示「只刷這個知識點」，避免點到沒有對應題目的空 session。
- 原題、聽力、答案、圖片、音檔、Learning、localStorage key 均不變。
